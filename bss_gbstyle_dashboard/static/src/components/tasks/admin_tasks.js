...
export class AdminTasks extends Component {
    static template = "bss_gbstyle_dashboard.AdminTasks";
    static props = {};
    ...
    setup() {
        ...
        // Calculate To Month (currentMonth + 1), keeping To Year as Current Year
        // If currentMonth + 1 > 12, cap it at 12 (same month as From)
        const toYear = currentYear;
        const toMonth = Math.min(currentMonth, 12);
        ...
        onWillStart(async () => {
            ...
            await this.getAllTasks(
                ...
                parseInt(this.state.manager_id)
            );
    ...
    async getAllTasks(name, year, month, toYear, toMonth, team_id, manager_id) {
        ...
        this.state.tasks = await this.orm.call(this.model, "get_admin_dashboard_data", [this.model, name, year, month, toYear, toMonth, team_id, manager_id]);
        /*
    ...
    async searchTasks(e) {
        if (
            ...
            // NOTE: 'name' is undefined in this scope. Assuming you want to search for this.nameRef?.el?.value
    ...
    getYearListUntilToday() {
        ...
        for (let year = startYear; year <= currentYear; year++) {
            years.push(String(year)); // Convert to string to match state values
        }
    ...
    async getTeams() {
        this.state.teams = await this.orm.call(this.model, "get_teams", [this.model]);
    }
    async filterByTeam() {
        let name = this.nameRef?.el?.value?.trim() || '';
        await this.getAllTasks(
    ...
    async getManagers() {
        this.state.managers = await this.orm.call(this.model, "get_managers", [this.model]);
    }
